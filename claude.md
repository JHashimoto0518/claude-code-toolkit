...
作成要否はユーザーが判断する。作成する場合は `/steering-new [開発タイトル]` スキルを使う(ファイル構成・命名規則・作成手順は `.claude/skills/steering-new/SKILL.md` を参照)。
...
初回実装についてステアリングドキュメントを作成する場合は `/steering-new` コマンドを使う。
...
作成要否はユーザーが判断する。作成する場合は `/steering-new [開発タイトル]` スキルを使う(`.claude/skills/steering-new/SKILL.md` 参照)。このスキル内で requirements.md → design.md → 永続的ドキュメント（`docs/`）更新 の順に段階承認しながら進め、承認後に tasklist.md を作成する。永続的ドキュメントの更新はこのステップ内(実装着手前)で完了させる。
...
- Stop フック(`.claude/hooks/stop-run-tests.sh`)がこのファイルを読んでテストを実行し、失敗した場合はターンの終了をブロックする
...
Claude Code の権限は `.claude/settings.json` の `permissions` と `.claude/hooks/pretooluse-block-prohibited.sh` で定義する。運用ルールは次の 4 つ。
...
4. **禁止事項は Git 管理で永続化する** - `.claude/settings.json` と `.claude/hooks/` に置き、開発コンテナの再作成後も維持する。`.claude/settings.local.json` はグローバル無視されるため恒久ルールの置き場所にしない
...
- `.claude/hooks/` - 禁止を担うスクリプトと、毎ターン実行される Stop / PostToolUse スクリプト
...
`.claude/skills/` と `.claude/commands/` は拒否しない。スキルの改訂は本リポジトリの通常の開発行為であり、Git の差分でレビューできるため。
...
- `.claude/settings.json` / `.claude/hooks/` / `.devcontainer/` の変更が必要になった場合は、Claude が編集せずユーザーに依頼する
