...
## 問題判明(配布先リポジトリでのコンテナ再作成確認)

上記の実地確認はこのリポジトリでのコンテナ再作成に限られており、配布先リポジトリでのコンテナ再作成までは検証できていなかった。

実際に配布先リポジトリの開発コンテナを作り直したところ、このリポジトリでは再作成後もプラグインが有効なままだったのに対し、配布先リポジトリでは再作成によりプラグインが失われることが判明した。原因を調査し、対応する必要がある。
